...
# 
# This script anaylzes normalized TIMBR predictions generated using
# the rat metabolic network and gene expression changes 
# from rat hepatocytes treated with various 
# pharmaceutical compounds and environmental toxicants.
#
# Code Adapted from blais_timbr_analysis.R which can be found at 
...
#path.manuscript.directory = "C:/Users/kr2up/Documents/ncomm_blais_submission_113016/"
#path.timbr.directory = "C:/Users/kr2up/Documents/timbrrawproductionscores/"
...
hep.raw.metabolomics <- hep.raw.metabolomics[,c(2:65)] %>% rename(met = "met_name")
...
hep.raw.metabolomics$stdevs <- rowSds(data.matrix(hep.raw.metabolomics[,c(2:55)]))

# Calculate t stats for each metabolite 
n <- length(hep.raw.metabolomics$stdevs)
...
for (i in 1:n) {
  t_apap1[i,1] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(5:8)]),data.matrix(hep.raw.metabolomics[i,c(1:4)]), alternative = "two.sided", var.equal = FALSE)$statistic %>% unlist() %>% unname()  
  t_apap1[i,2] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(5:8)]),data.matrix(hep.raw.metabolomics[i,c(1:4)]), alternative = "two.sided", var.equal = FALSE)$p.value %>% unlist() %>% unname()  
  
  t_apap2[i,1] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(46:49)]),data.matrix(hep.raw.metabolomics[i,c(37:40)]), alternative = "two.sided", var.equal = FALSE)$statistic %>% unlist() %>% unname()
  t_apap2[i,2] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(46:49)]),data.matrix(hep.raw.metabolomics[i,c(37:40)]), alternative = "two.sided", var.equal = FALSE)$p.value %>% unlist() %>% unname()
  
  t_ccl41[i,1] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(9:12)]),data.matrix(hep.raw.metabolomics[i,c(1:4)]), alternative = "two.sided", var.equal = FALSE)$statistic %>% unlist() %>% unname() 
  t_ccl41[i,2] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(9:12)]),data.matrix(hep.raw.metabolomics[i,c(1:4)]), alternative = "two.sided", var.equal = FALSE)$p.value %>% unlist() %>% unname() 
  
  t_ccl42[i,1] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(50:55)]),data.matrix(hep.raw.metabolomics[i,c(41:45)]), alternative = "two.sided", var.equal = FALSE)$statistic %>% unlist() %>% unname()
  t_ccl42[i,2] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(50:55)]),data.matrix(hep.raw.metabolomics[i,c(41:45)]), alternative = "two.sided", var.equal = FALSE)$p.value %>% unlist() %>% unname()
  
  t_tcdd1[i,1] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(13:16)]),data.matrix(hep.raw.metabolomics[i,c(1:4)]), alternative = "two.sided", var.equal = FALSE)$statistic %>% unlist() %>% unname()
  t_tcdd1[i,2] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(13:16)]),data.matrix(hep.raw.metabolomics[i,c(1:4)]), alternative = "two.sided", var.equal = FALSE)$p.value %>% unlist() %>% unname()
  
  t_tcdd2[i,1] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(25:28)]),data.matrix(hep.raw.metabolomics[i,c(21:24)]), alternative = "two.sided", var.equal = FALSE)$statistic %>% unlist() %>% unname()
  t_tcdd2[i,2] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(25:28)]),data.matrix(hep.raw.metabolomics[i,c(21:24)]), alternative = "two.sided", var.equal = FALSE)$p.value %>% unlist() %>% unname()
  
   t_tce1[i,1] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(17:20)]),data.matrix(hep.raw.metabolomics[i,c(1:4)]), alternative = "two.sided", var.equal = FALSE)$statistic %>% unlist() %>% unname()  
   t_tce1[i,2] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(17:20)]),data.matrix(hep.raw.metabolomics[i,c(1:4)]), alternative = "two.sided", var.equal = FALSE)$p.value %>% unlist() %>% unname()  
   
    t_tce2[i,1] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(29:32)]),data.matrix(hep.raw.metabolomics[i,c(21:24)]), alternative = "two.sided", var.equal = FALSE)$statistic %>% unlist() %>% unname()  
    t_tce2[i,2] <-  t.test(data.matrix(hep.raw.metabolomics[i,c(29:32)]),data.matrix(hep.raw.metabolomics[i,c(21:24)]), alternative = "two.sided", var.equal = FALSE)$p.value %>% unlist() %>% unname()  
}
...
normal.hep.metabolomics$met <- mets
...
# Use GGPlot 2 to create heatmap 
normal.bin.metabolomics.melt <- melt(normal.bin.metabolomics,variable.names = "Condition", value.name = "Z-Score")

normal.bin.metabolomics.melt %>% ggplot(aes(met,variable)) + geom_tile(aes(fill =`Z-Score`),color = "grey99",size=0.001) + 
...
metab.hc <- hclust(dist(normalized.met.hmap, method = "euclidean"))
ggdendrogram(metab.hc, rotate = TRUE, theme_dendro = TRUE)+ 
  ggtitle("Metabolomic Clustering") + 
...
new_vald$stats <- factor(new_vald$stats, levels = c("no_change.no_change", "increase.increase", "decrease.decrease",
      "no_change.increase","no_change.decrease","increase.no_change","decrease.no_change","increase.decrease","decrease.increase"))
...
met_pos <- data.frame(matrix(0,ncol = 6,nrow = 31)) # Change 16 for unique metabolites 
...
newdrugs <- data.frame(drug_id.x = rep(condition_vector,each = 31)) # change to number of unique metabolites
...
f5.1 <- ggplot(metaframe,aes(x = x, y = y)) +
  ...
  scale_y_continuous(limits = c(-1,93),breaks = seq(0,90,3), labels = sort(unique(new_vald$met), decreasing = TRUE),expand = c(0,0)) + 
...
f5.2 <- new_vald %>% ggplot(aes(x = stats)) +
  ...
  geom_text(stat='count', aes(label = ..count..), hjust = -0.5) + ylim(0,150) +
...
ggsave('TIMBR_Figure.png', plot = last_plot(), width=6,height=10,units = "in",dpi = 300)
